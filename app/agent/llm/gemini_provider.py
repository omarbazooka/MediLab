...
class GeminiProvider:
    ...
    def understand_request(
        ...
        system_instruction = (
            ...
            "- customer_name: Full name provided by the customer for booking.\n"
            "- customer_phone: Mobile phone number provided by the customer.\n"
            "- scheduled_date: Appointment date requested (YYYY-MM-DD).\n"
            "- scheduled_time: Appointment time requested (HH:MM).\n"
            "- address / area: Detailed address and area for home visits.\n"
            "- booking_reference: Booking reference code (MLB-YYYYMMDD-XXXXXXXX).\n"
    ...
    def compose_response(
        ...
        system_instruction = (
            ...
            "If action_result is present in evidence_bundle, phrase the response strictly reflecting action_result.status:\n"
            "- If status is NEEDS_DATA, ask politely for the missing required field(s) (such as name or phone).\n"
            "- If status is SLOT_ERROR or SLOT_FULL, state clearly that the requested slot is unavailable or invalid and offer the exact alternative slots in action_result.alternatives.\n"
            "- If status is AWAITING_CONFIRMATION, summarize the appointment details (service, branch/address, date, exact time, total price) and ask the user to explicitly confirm.\n"
            "- If status is EXECUTED, congratulate the user, stating the real booking_reference, scheduled date, and exact time.\n"
            "- If status is CANCELLED, confirm that the booking has been cancelled and the slot released.\n"
            "- If status is STATUS_FOUND, report the real current booking status and appointment details.\n"
            "- If status is STATUS_NOT_FOUND, state that no booking was found for the supplied reference.\n"
            "Never claim an action succeeded unless action_result has committed=true and success=true.\n"
